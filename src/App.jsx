...
//  Helper to calculate Sun Exposure for the "Cool Route"
const calculateSunExposure = (routeCoords, obstacles) => {
  ...
  if (sunIntensity === 0) return 0; // Night time = 0 exposure

  let shadedPoints = 0;
  const threshold = 0.0005; // ~50m proximity to trees
  ...
  // Return percentage of route that is SUNNY (1.0 = all sun, 0.0 = all shade)
...
export default function App() {
  ...
  const calculateRoute = async () => {
    ...
    try {
      ...
      // Fetch all three simultaneously, select later based on mode the user wants
      ...
      // CASE A: SUMMER/COOL MODE
      if (isSummerMode) {
        // We prefer the ALTERNATIVE bike path (Index 1) if available, as Index 0 is often the main road.
        if (shadeRes && shadeRes.paths && shadeRes.paths.length > 0) {
          // Try to grab the 2nd path (Index 1) for maximum difference. If not, fallback to Index 0.
          ...
          // OPTIONAL: Fetch Tree Data for Score Display
      ...
      // CASE B: ORIGINAL LOGIC (SAFE VS WILD)
      else if (!isWellLit) {
        // === NON-SAFE MODE (Direct/Wild) ===
        ...
        if (cityRes && cityRes.paths && cityRes.paths.length > 0) {
          ...
          // 1. Define Bounding Box for the Route
          const minLat = Math.min(currentLocation.lat, dCoords.lat) - 0.01;
          const maxLat = Math.max(currentLocation.lat, dCoords.lat) + 0.01;
          const minLng = Math.min(currentLocation.lng, dCoords.lng) - 0.01;
          const maxLng = Math.max(currentLocation.lng, dCoords.lng) + 0.01;

          // 2. Query Overpass for Street Lamps & Lit Streets
          ...
          // 3. Fetch Data & Calculate Score
          fetch('https://overpass-api.de/api/interpreter', { method: 'POST', body: query })
            .then(r => r.json())
            .then(data => {
               // Convert GraphHopper coords to [lat, lng]
               const coords = selectedPath.points.coordinates.map(c => [c[1], c[0]]);
               
               const litCount = calculateLightingScore(coords, data.elements || []);
               
               // Calculate Percentage
               // calculateLightingScore samples every 10th point, so we divide total length by 10
              ...
              <div className="px-6 pb-4 shrink-0">
                <div className="flex p-1 bg-gray-900 rounded-xl">
                  <button
                    ...
                    className={`flex-1 py-2 rounded-lg text-sm font-medium transition-colors ${viewMode === 'rider' ? 'bg-blue-600 text-white' : 'text-gray-400'
                      }`}
                  >
                    Rider
                  </button>
                  ...
                  <button
                    ...
                    className={`flex-1 py-2 rounded-lg text-sm font-medium transition-colors ${viewMode === 'watcher' ? 'bg-purple-600 text-white' : 'text-gray-400'
                      }`}
                  >
                    Watcher
                  </button>
                      ...
                      <div className="flex gap-2">
                        ...
                        <div className="flex-1 flex items-center justify-between bg-gray-900 p-3 rounded-xl border border-gray-600/50 cursor-pointer"
                          onClick={() => {
                            ...
                            if (newState) setIsSummerMode(false); // Turn off Summer if Safe is ON
                          }}>
                          <div className="flex items-center gap-2">
                            <Moon size={18} className={isWellLit ? "text-cyan-400" : "text-gray-500"} />
                            <div className="flex flex-col">
                              <span className="text-xs font-bold">Safe</span>
                              <span className="text-[10px] text-gray-500 leading-tight">Well-lit</span>
                            </div>
                          </div>
                        ...
                        {/* 2. SUMMER/COOL TOGGLE */}
                        <div className="flex-1 flex items-center justify-between bg-gray-900 p-3 rounded-xl border border-gray-600/50 cursor-pointer"
                          onClick={() => {
                            const newState = !isSummerMode;
                            setIsSummerMode(newState);
                            if (newState) setIsWellLit(false); // Turn off Safe if Summer is ON
                          }}>
                          <div className="flex items-center gap-2">
                            <Sun size={18} className={isSummerMode ? "text-orange-400" : "text-gray-500"} />
                            <div className="flex flex-col">
                              <span className="text-xs font-bold">Cool</span>
                              <span className="text-[10px] text-gray-500 leading-tight">Shady</span>
                            </div>
                          </div>
